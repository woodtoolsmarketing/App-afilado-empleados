-- Sube el tope de buscar_articulos a 1000. La familia mas grande del catalogo
-- (fresa, 339) queda con margen amplio para la precarga del buscador, que baja
-- la familia entera y filtra en el cliente. Los llamadores normales siguen
-- pasando 20/40, asi que no cambia su comportamiento.
...
AS $function$
  ...
  order by
    ...
    c.codigo
  limit least(coalesce(p_limite, 20), 1000);
$function$;
