-- =============================================================================
-- WoodTools · Paso 2
-- El buscador de clientes también busca por CUIT
--
-- El encabezado de la nota de pedido tiene tres campos de identificación —Cod.
-- Cliente, Nombre y CUIT— y completar cualquiera de los tres tiene que llenar
-- los otros dos: el vendedor se acuerda de uno u otro según el caso, y
-- obligarlo a saber cuál de antemano sería trabajo suyo para comodidad nuestra.
-- =============================================================================

-- La firma cambia (se agregan cuit, email, vendedor_id), asi que hay que
-- borrar la version anterior antes de recrearla.
drop function if exists public.buscar_clientes(text, int);

-- El encabezado de la nota de pedido busca por Cod. Cliente, Nombre O CUIT, y
-- completar cualquiera de los tres tiene que llenar los otros dos.
...
comment on function public.buscar_clientes is
  'Buscador de clientes por codigo, razon social, nombre de fantasia o CUIT (con o sin guiones). Devuelve la direccion principal para autocompletar.';
